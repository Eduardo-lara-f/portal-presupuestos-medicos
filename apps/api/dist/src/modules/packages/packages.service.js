...
let PackagesService = class PackagesService {
    ...
    async validateProcedures(divisionId, procedureIds) {
        if (!procedureIds.length) {
            throw new common_1.BadRequestException('El paquete debe tener al menos una prestación.');
        }
    ...
    async create(dto) {
        ...
        if (existing) {
            throw new common_1.ConflictException('Ya existe un paquete con ese código en la división seleccionada.');
        }
            ...
            data: {
                ...
                items: {
                    create: dto.items.map((item) => ({
                        ...
                        fixedPrice: item.fixedPrice !== undefined ? item.fixedPrice : null,
                    })),
                },
            },
            include: {
    ...
    async update(id, dto) {
        ...
        if (duplicated) {
            throw new common_1.ConflictException('Ya existe otro paquete con ese código en la división seleccionada.');
        }
        if (dto.items) {
